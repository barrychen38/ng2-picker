...
export class BScroll extends EventEmitter {
  ...
  private snapThresholdX: number;
  private snapThresholdY: number;
  ...
  private directionLocked: any;
  private isInTransition: any;
  private selectedIndex: number;
  private resizeTimeout: any;
  private probeTimer: any;
  private hasHorizontalScroll: any;
  private hasVerticalScroll: any;
  private wrapperOffset: any;
	...
	_transitionTime(time = 0) {
		...
		if (this.options.wheel && !isBadAndroid) {
			for (let item of this.items) {
				item.style[style.transitionDuration] = time + 'ms';
			}
		}
	...
	_transitionTimingFunction(easing) {
		...
		if (this.options.wheel && !isBadAndroid) {
			for (let item of this.items) {
				item.style[style.transitionTimingFunction] = easing;
			}
		}
	}
	...
	refresh() {
		...
		this.wrapperWidth = parseInt(this.wrapper.style.width, 10) || this.wrapper.clientWidth;
		this.wrapperHeight = parseInt(this.wrapper.style.height, 10) || this.wrapper.clientHeight;

		this.scrollerWidth = parseInt(this.scroller.style.width, 10) || this.scroller.clientWidth;
		this.scrollerHeight = parseInt(this.scroller.style.height, 10) || this.scroller.clientHeight;
		if (this.options.wheel) {
	...
	getComputedPosition() {
		...
		if (this.options.useTransform) {
			newMatrix = matrix[style.transform].split(')')[0].split(', ');
			x = +(newMatrix[12] || newMatrix[4]);
			y = +(newMatrix[13] || newMatrix[5]);
		} else {
			x = +newMatrix.left.replace(/[^-\d.]/g, '');
			y = +newMatrix.top.replace(/[^-\d.]/g, '');
		}
